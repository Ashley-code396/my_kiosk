...
public fun lock_item_in_kiosk(kiosk: &mut Kiosk, cap: &KioskOwnerCap, _policy: &TransferPolicy<Item>,ctx: &mut TxContext){
    let item = create_item(ctx);
...
#[allow(lint(self_transfer))]
public fun take_item_from_kiosk(kiosk: &mut Kiosk, cap: &KioskOwnerCap, item_id: ID,ctx: &mut TxContext){
...
public fun list_with_purchase_cap(kiosk: &mut Kiosk, cap: &KioskOwnerCap, item_id: ID,min_price: u64, allowed_buyer: address, ctx: &mut TxContext){
...
#[allow(lint(self_transfer))]
public fun purchase_item_from_kiosk(kiosk: &mut Kiosk, item_id: ID, policy: &TransferPolicy<Item>,payment: Coin<SUI>, ctx: &mut TxContext){
...
#[allow(lint(self_transfer))]
public fun purchase_item_from_kiosk_with_purchase_cap(kiosk: &mut Kiosk,purchase_cap: PurchaseCap<Item>,policy: &TransferPolicy<Item>,payment: Coin<SUI>,ctx: &mut TxContext){
